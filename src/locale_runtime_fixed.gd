...
func translate_puzzle_text(puzzle_id: String, fallback: String) -> String:
    if language == "uz":
        return fallback
    return PuzzleTranslationsFallback(puzzle_id, fallback)

func PuzzleTranslationsFallback(puzzle_id: String, fallback: String) -> String:
    return str(PUZZLE_TRANSLATIONS.get(puzzle_id, language, fallback))
