...
func record_solved(seconds: float, category: String = "") -> void:
    ...
    _record_category(category, true, seconds)

func record_failed(category: String = "") -> void:
    failed += 1
...
func get_average_time() -> float:
    var attempts := solved + failed
    return 0.0 if attempts == 0 else total_time_seconds / float(attempts)
...
func to_dict() -> Dictionary:
    return {"solved": solved, "failed": failed, "hints": hints, "total_time_seconds": total_time_seconds, "category_stats": category_stats.duplicate(true)}
