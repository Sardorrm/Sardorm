...
const MIN_TOUCH_TARGET_HEIGHT := 64
const LEVEL_TOUCH_TARGET := Vector2(72, MIN_TOUCH_TARGET_HEIGHT)
...
func _button(text: String, callback: Callable, height: int = MIN_TOUCH_TARGET_HEIGHT) -> Button:
    var button := Button.new()
    button.text = text
    button.custom_minimum_size = Vector2(0, maxi(MIN_TOUCH_TARGET_HEIGHT, height))
...
func _show_levels() -> void:
    ...
    for i in range(engine.puzzles.size()):
        ...
        level_button.custom_minimum_size = LEVEL_TOUCH_TARGET
...
func _render_puzzle(puzzle: PuzzleDefinition, progress_text: String) -> void:
    ...
    content.add_child(_button("HINT", func(): _use_hint(hint), MIN_TOUCH_TARGET_HEIGHT))
    pause_button = _button("⏸ PAUZA", _toggle_pause, MIN_TOUCH_TARGET_HEIGHT)
    content.add_child(pause_button)
    content.add_child(_button("DARAJALAR" if not daily_mode else "CHALLENGE", _show_levels if not daily_mode else _show_daily, MIN_TOUCH_TARGET_HEIGHT))
    timer_tick.start()
...
func _build_text_answer(feedback: Label, hint: Label) -> void:
    ...
    answer.custom_minimum_size = Vector2(0, MIN_TOUCH_TARGET_HEIGHT)
...
func _build_button_answers(feedback: Label, hint: Label) -> void:
    for value in AnswerInputFactory.button_labels(active_puzzle):
        ...
        content.add_child(_button(value_text, func(): _submit_answer(value_text, null, feedback, hint), MIN_TOUCH_TARGET_HEIGHT))
