...
def test_ui_polish_avoids_per_frame_recursive_tree_scan():
    ...
    assert "_refresh_safe_area_if_needed()" in process


def test_ui_polish_caches_ui_and_margin_references():
    code = (ROOT / "src" / "ui_polish.gd").read_text(encoding="utf-8")
    assert "var _ui: Control" in code
    assert "var _margin: MarginContainer" in code
    assert "_bind_ui_nodes(node)" in code
    assert "_margin = _ui.get_node_or_null(\"MarginContainer\") as MarginContainer" in code
...
def test_ui_polish_refreshes_safe_area_only_when_layout_changes():
    ...
    assert "if not force and _ui.size == _last_ui_size and screen == _last_screen_size:" in code
