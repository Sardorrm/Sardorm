...
def test_ui_polish_uses_lifecycle_signals_for_dynamic_nodes():
    ...
    assert "call_deferred(\"_refresh_current_scene\")" in code
...
def test_ui_polish_does_not_schedule_scene_wide_refresh_for_every_added_node():
    code = (ROOT / "src" / "ui_polish.gd").read_text(encoding="utf-8")
    node_added = code.split("func _on_node_added(node: Node) -> void:", 1)[1].split("func _on_node_removed", 1)[0]
    assert "call_deferred(\"_refresh_current_scene\")" not in node_added
    assert "_bind_ui_nodes(node)" in node_added
